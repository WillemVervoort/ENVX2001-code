...
The se_fit for the prediction interval can be calculated as `(fit - lwr)/qt(0.975,(nrow(Corn)-1))`
```
...
\begin{center}
$BIAS = \frac{\sqrt{\sum^n_{i-1}(y_i - \hat{y}_i)}}{n}$
\end{center}
...
(bias <- sqrt(sum(newC[,1] - newCornP_se[,1]))/5)
```
...
The model model is fairly accurate as a RMSE of `r RMSE` is only small compared to the range of CornP in the data `r range(Corn$CornP)`
```

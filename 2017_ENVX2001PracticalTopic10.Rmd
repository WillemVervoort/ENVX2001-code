...
More interesting is making prediction not part of the original data, but as we discussed in the lectures this means there is a different confidence interval. R allows you to define the interval using `interval = "prediction"`. The output will then include both the fitted and the prediction confidence interval and the default is to calculate the 95% confidence interval. If you want to calculate the actual se.fit from the output, you need to subtract the actual prediction and divide by the t~0.05~ for `df = 43`.  
...
d. To calculate the true se.fit outside the confidence intervals, subtract column 1 from column 3 and divide by 2-tailed t for the interval at df 42 (which is `r round(qt(0.975,42),2)`).  

```{asis, eval = F}
####Answer
```
```{r pred_se_Lake, echo = F, eval= F}
predict.se <- (Lake_predict[,3] - Lake_predict[,1])/qt(0.975,42)
predict.se
```
